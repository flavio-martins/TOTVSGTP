...
If ( !FindFunction("GTPHASACCESS") .Or.; 
    ...
    aRotina     := MenuDef()
    aNewFlds    := {'GQ8_DESCRI','GQ8_OBSERV','GQ8_CODG6R','GQ8_CODGY0','GQ8_CODGYD'}
...
ADD OPTION aRotina TITLE "Relatorio"    ACTION 'GTPR901' OPERATION 6 ACCESS 0 // Relatorio
...
Local aNewFlds  := {'GQ8_DESCRI', 'GQ8_OBSERV','GQ8_CODG6R','GQ8_CODGY0','GQ8_CODGYD'}
...
Else
    ...
    oStruGQ8	:= FWFormStruct(1,'GQ8',,.F. )
    oStruGQB	:= FwFormStruct(1,'GQB',,.F. )

    SetModelStruct(oStruGQ8,oStruGQB)
...
/*/
Static Function SetModelStruct(oStruGQ8,oStruGQB)

	Local bFldWhen := {|oModel,cField,uVal|FieldWhen(oModel,cField,uVal) }
//	Local bFldVal  := {|oModel,cField,uNewValue,uOldValue|FieldVal(oModel,cField,uNewValue,uOldValue) }
...
/*
    oStruGQ8:SetProperty( 'GQ8_CODGY0', MODEL_FIELD_VALID, bFldVal)
    oStruGQ8:SetProperty( 'GQ8_CODGYD', MODEL_FIELD_VALID, bFldVal)
    oStruGQ8:SetProperty( 'GQ8_CODG6R', MODEL_FIELD_VALID, bFldVal)
*/
    oStruGQ8:SetProperty( 'GQ8_CODGY0', MODEL_FIELD_WHEN, bFldWhen)
    oStruGQ8:SetProperty( 'GQ8_CODGYD', MODEL_FIELD_WHEN, bFldWhen)
    oStruGQ8:SetProperty( 'GQ8_CODG6R', MODEL_FIELD_WHEN, bFldWhen)
...
Local oStruGQ8	:= nil
Local oStruGQB	:= nil
Local aNewFlds   := {'GQ8_DESCRI', 'GQ8_OBSERV','GQ8_CODGY0','GQ8_CODGYD','GQ8_CODG6R'}
Local aNewGqb    := {'GQB_FILIAL', 'GQB_CODIGO','GQB_ITEM','GQB_NOME','GQB_CPF','GQB_CEP',;
                     'GQB_ENDERE','GQB_COMPLE','GQB_BAIRRO','GQB_MUNICI','GQB_ESTADO'}
...
Else
    ...
    oStruGQ8	:= FWFormStruct(2, 'GQ8',,.F. )
    oStruGQB	:= FWFormStruct(2, 'GQB',,.F. )
...
If oModel:VldData()
    ...
    If !(FwIsInCallStack('GTPA901ORC'))
        If (!Empty(oModel:GetValue('GQ8MASTER','GQ8_CODGYD')) .or. !Empty(oModel:GetValue('GQ8MASTER','GQ8_CODG6R')) .or. !Empty(oModel:GetValue('GQ8MASTER','GQ8_CODGY0')))
            lRet := FWFormCommit(oModel)
        Else
            lRet      := .F.
            oModel:SetErrorMessage(oModel:GetId(),,oModel:GetId(),,,"Falta digitação de dados", "Algum dos campos de identificação deve ser digitado. (Simulação, Contrato ou Linha)",,)
        EndIf
    Else
        oMdl901 := FwLoadModel('GTPA901')
        oMdl901:SetOperation(MODEL_OPERATION_INSERT)
...
Static Function FieldWhen(oMdl,cField,uVal)
    ...
    If oModel:GetOperation() == MODEL_OPERATION_INSERT
        lRet      := .T.
    	Do Case
        	Case cField == "GQ8_CODGY0"
                If (!Empty(oModel:GetValue('GQ8MASTER','GQ8_CODGYD')) .or. !Empty(oModel:GetValue('GQ8MASTER','GQ8_CODG6R'))) .and. oModel:GetOperation() == MODEL_OPERATION_INSERT
		            lRet := .F.
                ElseIf (Empty(oModel:GetValue('GQ8MASTER','GQ8_CODGYD')) .and. Empty(oModel:GetValue('GQ8MASTER','GQ8_CODG6R'))) .and. oModel:GetOperation() == MODEL_OPERATION_UPDATE
                    lRet := .T.
                EndIf
            Case cField == "GQ8_CODGYD"
                If (!Empty(oModel:GetValue('GQ8MASTER','GQ8_CODGY0')) .or. !Empty(oModel:GetValue('GQ8MASTER','GQ8_CODG6R'))) .and. oModel:GetOperation() == MODEL_OPERATION_INSERT
		            lRet := .F.
                ElseIf (Empty(oModel:GetValue('GQ8MASTER','GQ8_CODGY0')) .and. Empty(oModel:GetValue('GQ8MASTER','GQ8_CODG6R'))) .and. oModel:GetOperation() == MODEL_OPERATION_UPDATE
                    lRet := .T.                
                EndIf
            Case cField == "GQ8_CODG6R" 
                If (!Empty(oModel:GetValue('GQ8MASTER','GQ8_CODGY0')) .or. !Empty(oModel:GetValue('GQ8MASTER','GQ8_CODGYD'))) .and. oModel:GetOperation() == MODEL_OPERATION_INSERT
		            lRet := .F.
                ElseIf (Empty(oModel:GetValue('GQ8MASTER','GQ8_CODGY0')) .and. Empty(oModel:GetValue('GQ8MASTER','GQ8_CODGYD'))) .and. oModel:GetOperation() == MODEL_OPERATION_UPDATE
                    lRet := .T.                
                EndIf
        EndCase
...
Static Function FieldVal(oMdl,cField,uNewValue,uOldValue)
    ...
    Do CASE
    	Case cField == "GQ8_CODGY0"
		    If !ExistCPO( "GY0", FWxFilial("GY0")+FWFldGet("GQ8_CODGY0") )
                lRet := .F.
            EndIf
        Case cField == "GQ8_CODG6R"
		    If !ExistCPO( "G6R", FWxFilial("G6R")+FWFldGet("GQ8_CODG6R") )
                lRet := .F.
            EndIf
        Case cField == "GQ8_CODGYD"
		    If !ExistCPO( "GYD", FWxFilial("GYD")+FWFldGet("GQ8_CODGYD") )
                lRet := .F.
            EndIf
    EndCase
A := 1
